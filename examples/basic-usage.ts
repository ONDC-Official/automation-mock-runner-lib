...
async function runONDCExample() {
  ...
  try {
    ...
    const searchResult = await runner.runGeneratePayload("search_electronics", searchInputs);
    ...
    const searchValidation = await runner.runValidatePayload("search_electronics", mockSearchResponse);
    if (searchValidation.success) {
      console.log("✅ Search response validation passed!");
      console.log("📋 Validation result:", searchValidation.result);
    } else {
      console.error("❌ Search response validation failed:", searchValidation.error);
    }
    ...
    const selectRequirements = await updatedRunner.runMeetRequirements("select_item", {});
    ...
    const selectResult = await updatedRunner.runGeneratePayload("select_item", selectInputs);
